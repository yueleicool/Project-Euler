...
(define (main args)
  (begin
    ...
    (newline)
    (display (format #f "case (6) exp=4, n=16*M, M odd prime, 1<=M<max-num/16, only solutions where (u=4, v=4M) : condition 1 is (u+v)/4 = (4+4M)/4 an integer, and (3v-u)/4 = (12M-4)/4 an integer.  The other possibilities fail condition 1 (u+v)/4 an integer.~%"))
    (newline)
