...
;;;#############################################################
;;;#############################################################
(define (main-loop max-prime max-nn debug-flag)
  (let ((sum-primes 10)
	(primes-count 3)
	...
	(do ((ii 3 (1+ ii)))
	  ...
	  (begin
	    (let ((ii-prime (array-ref prime-array ii)))
	      (begin
		(let ((rr-remain (modulo-expt 10 max-nn ii-prime)))
		  (begin
		    (if (not (= rr-remain 1))
			(begin
			  ...
			  (if (equal? debug-flag #t)
			      (begin
				(display (ice9-format:format #f "  prime=~:d, sum-primes=~:d : (~:d / ~:d)~%"
							     ii-prime sum-primes primes-count plen))
				(force-output)
				))
			  ))
		    ))
	...
	(display (ice9-format:format #f "Sum(primes) = ~:d, there were ~:d / ~:d primes that will never be a factor of R(10^n), (for primes less than ~:d).~%"
				     sum-primes primes-count plen max-prime))
	(force-output)
	))
    ))
...
(define (main args)
  (begin
    (display (format #f "Project Euler 133 - A number consisting entirely of ones is called a repunit. We shall define R(k) to be a repunit of length k.~%"))
    (newline)
    (display (format #f "Let us consider repunits of the form R(10^n).~%"))
    (newline)
    (display (format #f "Although R(10), R(100), or R(1000) are not divisible by 17, R(10000) is divisible by 17. Yet there is no value of n for which R(10^n) will divide by 19. In fact, it is remarkable that 11, 17, 41, and 73 are the only four primes below one-hundred that can be a factor of R(10^n).~%"))
    (newline)
    (display (format #f "Find the sum of all the primes below one-hundred thousand that will never be a factor of R(10^n).~%"))
    (newline)
    (display (format #f "The solution makes use of the definition of R(k) = (10^k - 1)/9, and the modulo-expt function. It also assumes that if the prime doesn't divide R(10^20) then it will never divide any R(k).  This is because if a divides k (k = a*m), then R(a) divides R(k), since 10^a = (9R(a)+1) and R(k)=(10^k - 1)/9=(10^(a*m)-1)/9=((9R(a)+1)^m-1)/9, the 1's cancel on expansion, and every term in the expansion contains at least one power of R(a).  This means that if p divides R(10^n) for some n, then p also divides R(10^m) if n < m, since R(10^n) divides R(10^m).  This solution was found at http://blog.dreamshire.com/2009/06/16/project-euler-problem-133-solution/~%"))
    (newline)
    (force-output)
	...
	(time-code
	 (begin
	   (main-loop max-prime max-nn debug-flag)
	   ))
	))
