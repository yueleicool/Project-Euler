...
(define (palindrome? nn)
  (cond
   ...
   (else
    (begin
      (let ((plist (split-digits-list nn)))
...
(define (main-loop start-num end-num)
  (let ((max-palindrome 0)
	...
	(begin
      ...
      (if (> max-palindrome 0)
	  (begin
	    (display (ice9-format:format #f "largest palindrome found: ~:d * ~:d = ~:d, (between ~:d and ~:d)~%"
					 max-ii max-jj max-palindrome
					 start-num end-num)))
	  (begin
	    (display (ice9-format:format #f "no palindromes found between ~:d and ~:d~%"
					 start-num end-num))
	    ))
      (force-output)
      )))
...
(define (main args)
  (begin
    (display (format #f "Project Euler 4: A palindromic number reads the same both ways. The largest palindrome made from the product of two 2-digit numbers is 9009 = 91 x 99.~%"))
    (newline)
    (display (format #f "Find the largest palindrome made from the product of two 3-digit numbers.~%"))
