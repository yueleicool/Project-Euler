   ...
   (else
	...
	(do ((ii 5 (+ ii 2)))
	  ...
	  (begin
	    (if (prime? ii)
		(begin
		  (set! nth-prime-index (+ nth-prime-index 1))
...
;;;#############################################################
;;;#############################################################
(define (main-loop nth-index max-prime)
  (let ((nth-prime-value (find-nth-prime nth-index max-prime)))
    (begin
      (display (ice9-format:format #f "the ~:d prime is ~:d~%"
...
(define (main args)
  (begin
    (display (format #f "Project Euler 7: By listing the first six prime numbers: 2, 3, 5, 7, 11, and 13, we can see that the 6th prime is 13.~%"))
    (display (format #f "What is the 10,001 st prime number?~%"))
    (newline)
	...
	(time-code
	 (begin
	   ...
	   (run-test test-find-nth-prime-1 counter)
