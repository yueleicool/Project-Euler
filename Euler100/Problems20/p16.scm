...
;;;#############################################################
;;;#############################################################
(define (main-loop base exponent)
  (let ((tnum (integer-power base exponent)))
    (let ((dlist (split-digits-list tnum)))
      (let ((dsum (srfi-1:fold + 0 dlist)))
	(display (ice9-format:format #f "~:d^~:d = ~:d : sum of digits ~a = ~:d~%"
				     base exponent tnum
				     (list-to-sum-string dlist)
				     dsum))
	(force-output)
	))
    ))
...
(define (main args)
  (begin
    (display (format #f "Problem 016 - 2^15 = 32768 and the sum of its digits is 3 + 2 + 7 + 6 + 8 = 26.~%"))
    (newline)
    (display (format #f "What is the sum of the digits of the number 2^1000?~%"))
    (newline)
